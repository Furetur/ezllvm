...
let lookup_fn env (id : Ollvm.Ast.ident) : Llvm.llvalue =
  match id with
...
let dll_storage : Ollvm.Ast.dll_storage -> Llvm.Linkage.t =
  let open Llvm.Linkage in
  function
  | DLLSTORAGE_Dllimport -> Dllimport | DLLSTORAGE_Dllexport -> Dllexport

let visibility : Ollvm.Ast.visibility -> Llvm.Visibility.t =
  let open Llvm.Visibility in
  function
...
let rec typ : env -> Ollvm.Ast.typ -> Llvm.lltype =
 fun env ->
  ...
  | TYPE_Struct s -> struct_type ctx (Array.of_list s |> Array.map (typ env))
...
let conversion_type :
    Ollvm.Ast.conversion_type -> Llvm.llvalue -> Llvm.lltype -> Llvm.llvalue =
  let open Llvm in
  function
  | Trunc -> const_trunc
  | Zext -> const_zext
...
let rec value : env -> Ollvm.Ast.typ -> Ollvm.Ast.value -> Llvm.llvalue =
 fun env ty ->
  ...
  | VALUE_Struct s ->
      const_struct env.c
        (Array.of_list s |> Array.map (fun (ty, v) -> value env ty v))
  | VALUE_Packed_struct s ->
      const_packed_struct env.c
        (Array.of_list s |> Array.map (fun (ty, v) -> value env ty v))
  | VALUE_Array a ->
      const_array (typ env ty)
        (Array.of_list a |> Array.map (fun (ty, v) -> value env ty v))
  | VALUE_Vector v ->
...
let rec instr : env -> Ollvm.Ast.instr -> env * Llvm.llvalue =
 fun env ->
  ...
  | INSTR_GetElementPtr ((t, v), tvl) ->
      let indices =
        List.map (fun (t, v) -> value env t v) tvl |> Array.of_list
      in
  ...
  | INSTR_Phi (t, incoming) ->
      let incoming =
        List.map (fun (v, i) -> (value env t v, label env i)) incoming
      in
  ...
  | INSTR_Invoke ((t, i1), tvl, (_, i2), (_, i3)) ->
      let args = List.map (fun (t, v) -> value env t v) tvl |> Array.of_list in
      let fn = lookup_fn env i1 in
      (env, build_invoke fn args (label env i2) (label env i3) "" env.b)
  | INSTR_Ret (t, v) -> (env, build_ret (value env t v) env.b)
  | INSTR_Ret_void -> (env, build_ret_void env.b)
  ...
  | INSTR_Br_1 (_, i) -> (env, build_br (label env i) env.b)
  ...
  | INSTR_IndirectBr ((t, v), til) ->
      ...
      List.iter
        (fun (_, i) -> Llvm.add_destination indirectbr (label env i))
        til;
      (env, indirectbr)
  | INSTR_Resume (t, v) ->
...
let global : env -> Ollvm.Ast.global -> env =
 fun env g ->
  let llv =
    value env g.g_typ
      (match g.g_value with Some x -> x | None -> assert false)
  in
...
let definition : env -> Ollvm.Ast.definition -> env =
 fun env df ->
  ...
  if Array.length (Llvm.basic_blocks fn) <> 0 then assert false;
  let env =
    ...
    |> Array.fold_left
         (fun env (i, a) ->
           ...
           { env with mem = (i, a) :: env.mem })
         env
  in
  (* First create needed blocks.
   * [block] function will use them when building instructions. *)
  let env =
    List.fold_left (fun env b -> create_block env b fn) env df.df_instrs
  in
...
let modul : Ollvm.Ast.modul -> env =
 fun modul ->
  ...
  let env = { c; m; b; mem = []; labels = [] } in
  let env =
    List.fold_left
      (fun env g -> global { env with mem = [] } g)
      env
      (List.map snd modul.m_globals)
  in
  let env =
    List.fold_left
      (fun env dc -> fst (declaration { env with mem = [] } dc))
      env
      (List.map snd modul.m_declarations)
  in
  let env =
    List.fold_left
      (fun env df -> definition { env with mem = []; labels = [] } df)
      env
      (List.map snd modul.m_definitions)
  in
  { env with mem = []; labels = [] }
