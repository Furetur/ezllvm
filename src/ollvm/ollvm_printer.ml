...
let empty_env () =
  { local = (ref 0, Hashtbl.create 0); global = (ref 0, Hashtbl.create 0) }
...
let pp_sep str ppf () = pp_print_string ppf str
...
and ident : t -> Format.formatter -> Ollvm_ast.ident -> unit =
  let ident_format : (string -> int) -> Format.formatter -> string -> unit =
   fun finder ppf i ->
    ...
    else pp_print_string ppf i
  in

  fun env ppf -> function
    | ID_Global i ->
        pp_print_char ppf '@';
...
and typ : Format.formatter -> Ollvm_ast.typ -> unit =
 fun ppf -> function
  ...
  | TYPE_Function (t, tl) ->
      assert false (* (t, tl) : Format.formatter -> (typ * typ list) *)
  | TYPE_Struct tl ->
      fprintf ppf "{%a}" (pp_print_list ~pp_sep:pp_comma_space typ) tl
  | TYPE_Packed_struct tl ->
      fprintf ppf "<{%a}>" (pp_print_list ~pp_sep:pp_comma_space typ) tl
...
and fast_math ppf fast_math =
  pp_print_string ppf
    (match fast_math with
    | Nnan -> "nnan"
    | Ninf -> "ninf"
    | Nsz -> "nsz"
    | Arcp -> "arcp"
...
and instr : t -> Format.formatter -> Ollvm_ast.instr -> unit =
 fun env ppf -> function
  | INSTR_IBinop (op, t, v1, v2) ->
      fprintf ppf "%a %a %a, %a" ibinop op typ t (value env) v1 (value env) v2
  | INSTR_ICmp (c, t, v1, v2) ->
      fprintf ppf "icmp %a %a %a, %a" icmp c typ t (value env) v1 (value env) v2
  | INSTR_FBinop (op, f, t, v1, v2) ->
      fbinop ppf op;
      if f <> [] then (
        pp_space ppf ();
        pp_print_list ~pp_sep:pp_space fast_math ppf f);
      fprintf ppf " %a %a, %a" typ t (value env) v1 (value env) v2
  | INSTR_FCmp (c, t, v1, v2) ->
      fprintf ppf "fcmp %a %a %a, %a" fcmp c typ t (value env) v1 (value env) v2
  | INSTR_Conversion (c, t1, v, t2) ->
      fprintf ppf "%a %a %a to %a" conversion_type c typ t1 (value env) v typ t2
  | INSTR_GetElementPtr (tv, tvl) ->
      fprintf ppf "getelementptr %a, %a" (tvalue env) tv
        (pp_print_list ~pp_sep:pp_comma_space (tvalue env))
        tvl
  | INSTR_Call (ti, tvl) ->
      fprintf ppf "call %a(%a)" (tident env) ti
        (pp_print_list ~pp_sep:pp_comma_space (tvalue env))
        tvl
  | INSTR_Alloca (t, n, a) -> (
      fprintf ppf "alloca %a" typ t;
      (match n with None -> () | Some n -> fprintf ppf ", %a" (tvalue env) n);
      match a with None -> () | Some a -> fprintf ppf ", align %d" a)
  | INSTR_Load (vol, tv, a) -> (
      ...
      match a with None -> () | Some a -> fprintf ppf ", align %d" a)
  | INSTR_Phi (t, vil) ->
      fprintf ppf "phi %a [%a]" typ t
        (pp_print_list ~pp_sep:(pp_sep "], [") (fun ppf (v, i) ->
             value env ppf v;
             pp_print_string ppf ", ";
             ident env ppf i))
        vil
  | INSTR_Select (if_, then_, else_) ->
      fprintf ppf "select %a, %a, %a" (tvalue env) if_ (tvalue env) then_
        (tvalue env) else_
  | INSTR_VAArg -> pp_print_string ppf "vaarg"
  | INSTR_ExtractElement (vec, idx) ->
      fprintf ppf "extractelement %a, %a" (tvalue env) vec (tvalue env) idx
  | INSTR_InsertElement (vec, new_val, idx) ->
      fprintf ppf "insertelement %a, %a, %a" (tvalue env) vec (tvalue env)
        new_val (tvalue env) idx
  | INSTR_ExtractValue (agg, idx) ->
      fprintf ppf "extractvalue %a, %a" (tvalue env) agg
        (pp_print_list ~pp_sep:pp_comma_space pp_print_int)
        idx
  | INSTR_InsertValue (agg, new_val, idx) ->
      fprintf ppf "insertvalue %a, %a, %a" (tvalue env) agg (tvalue env) new_val
        (pp_print_list ~pp_sep:pp_comma_space pp_print_int)
        idx
  | INSTR_ShuffleVector (v1, v2, mask) ->
      fprintf ppf "shufflevector %a, %a, %a" (tvalue env) v1 (tvalue env) v2
        (tvalue env) mask
  ...
  | INSTR_Store (vol, v, ptr, a) -> (
      ...
      match a with None -> () | Some a -> fprintf ppf ", align %d" a)
  | INSTR_AtomicCmpXchg | INSTR_AtomicRMW | INSTR_Fence -> assert false
  ...
  | INSTR_Switch (c, def, cases) ->
      fprintf ppf "switch %a, %a [%a]" (tvalue env) c (tident env) def
        (pp_print_list ~pp_sep:pp_space (fun ppf (v, i) ->
             tvalue env ppf v;
             pp_print_string ppf ", ";
             tident env ppf i))
        cases
  ...
  | INSTR_IndirectBr (tv, til) ->
      fprintf ppf "indirectbr %a, [%a]" (tvalue env) tv
        (pp_print_list ~pp_sep:pp_comma_space (tident env))
        til
  | INSTR_Invoke (ti, tvl, i2, i3) ->
      fprintf ppf "invoke %a(%a) to %a unwind %a" (tident env) ti
        (pp_print_list ~pp_sep:pp_comma_space (tvalue env))
        tvl (tident env) i2 (tident env) i3
  | INSTR_Assign (id, inst) ->
      fprintf ppf "%a = %a" (ident env) id (instr env) inst
...
and value : t -> Format.formatter -> Ollvm_ast.value -> unit =
 fun env ppf -> function
  ...
  | VALUE_Array tvl ->
      fprintf ppf "[%a]" (pp_print_list ~pp_sep:pp_comma_space (tvalue env)) tvl
  | VALUE_Vector tvl ->
      fprintf ppf "<%a>" (pp_print_list ~pp_sep:pp_comma_space (tvalue env)) tvl
  | VALUE_Struct tvl ->
      fprintf ppf "{%a}" (pp_print_list ~pp_sep:pp_comma_space (tvalue env)) tvl
...
and toplevelentry : t -> Format.formatter -> Ollvm_ast.toplevelentry -> unit =
 fun env ppf -> function
  ...
  | TLE_Attribute_group (i, a) ->
      fprintf ppf "#%d = { %a }" i (pp_print_list ~pp_sep:pp_space fn_attr) a
...
and metadata : t -> Format.formatter -> Ollvm_ast.metadata -> unit =
 fun env ppf -> function
  ...
  | METADATA_Node m ->
      fprintf ppf "metadata !{%a}"
        (pp_print_list ~pp_sep:pp_comma_space (metadata env))
        m
  | METADATA_Named m ->
      fprintf ppf "!{%a}"
        (pp_print_list ~pp_sep:pp_comma_space (fun ppf i -> fprintf ppf "!%s" i))
        m
...
and global : t -> Format.formatter -> Ollvm_ast.global -> unit =
 fun env ppf
  ...
  fprintf ppf "%a = %s %a" (ident env) i
    (if b then "constant" else "global")
    typ t;
  (match vo with None -> () | Some v -> (value env) ppf v);
  (match s with None -> () | Some s -> fprintf ppf ", section %s" s);
  match a with None -> () | Some a -> fprintf ppf ", align %d" a
...
and declaration : t -> Format.formatter -> Ollvm_ast.declaration -> unit =
 fun env ppf
  ...
  if ret_attrs <> [] then (
    pp_space ppf ();
    pp_print_list ~pp_sep:pp_space param_attr ppf ret_attrs);
  fprintf ppf "%a %a(%a)" typ ret_t (ident env) i
...
and definition : t -> Format.formatter -> Ollvm_ast.definition -> unit =
 fun env ppf
  ...
  let typ_attr_id ppf ((t, attrs), id) =
    typ ppf t;
    pp_space ppf ();
    if attrs <> [] then (
      pp_print_list ~pp_sep:pp_space param_attr ppf attrs;
  ...
  if df.df_attrs <> [] then (
    pp_print_list ~pp_sep:pp_space fn_attr ppf df.df_attrs;
    pp_space ppf ());
  (match df.df_section with
  | Some x -> fprintf ppf "section \"%s\" " x
  | _ -> ());
  (match df.df_align with Some x -> fprintf ppf "align %d " x | _ -> ());
  (match df.df_gc with Some x -> fprintf ppf "gc \"%s\" " x | _ -> ());
  pp_print_char ppf '{';
...
and block : t -> Format.formatter -> Ollvm_ast.block -> unit =
 fun env ppf (i, b) ->
  (try pp_print_int ppf (find_local env i)
...
and modul : t -> Format.formatter -> Ollvm_ast.modul -> unit =
 fun env ppf m ->
  ...
  pp_print_list ~pp_sep:pp_force_newline
    (global (reset_local env))
    ppf (List.map snd m.m_globals);
  pp_force_newline ppf ();

  (* Print function declaration only if there is no corresponding
     function definition *)
  pp_print_list ~pp_sep:pp_force_newline
    (declaration (reset_local env))
    ppf
    (List.filter
