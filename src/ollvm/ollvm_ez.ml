...
module Value = struct
  ...
  let structure l = (Type.structure (List.map fst l), Ollvm_ast.VALUE_Struct l)
...
module Instr = struct
  ...
  let call ((t, _) as fn) args = (t, Ollvm_ast.INSTR_Call (ident fn, args))

  let phi value_label =
    let t = List.hd value_label |> fst |> fst in
    let value_label =
      List.map (fun (v, i) -> (snd v, snd (ident i))) value_label
    in
  ...
  let eq = icmp Ollvm_ast.Eq
  let ne = icmp Ollvm_ast.Ne
  let ugt = icmp Ollvm_ast.Ugt
  let uge = icmp Ollvm_ast.Uge
  let ult = icmp Ollvm_ast.Ult
  let ule = icmp Ollvm_ast.Ule
  let sgt = icmp Ollvm_ast.Sgt
  let sge = icmp Ollvm_ast.Sge
  let slt = icmp Ollvm_ast.Slt
  let sle = icmp Ollvm_ast.Sle

  let fcmp cmp (t, op1) (_, op2) =
    (Type.i1, Ollvm_ast.INSTR_FCmp (cmp, t, op1, op2))

  let ffalse = fcmp Ollvm_ast.False
  let foeq = fcmp Ollvm_ast.Oeq
  let fogt = fcmp Ollvm_ast.Ogt
  let foge = fcmp Ollvm_ast.Oge
  let folt = fcmp Ollvm_ast.Olt
  let fole = fcmp Ollvm_ast.Ole
  let fone = fcmp Ollvm_ast.One
  let ord = fcmp Ollvm_ast.Ord
  let fueq = fcmp Ollvm_ast.Ueq
  let fugt = fcmp Ollvm_ast.Ugt
  let fuge = fcmp Ollvm_ast.Uge
  let fult = fcmp Ollvm_ast.Ult
  let fule = fcmp Ollvm_ast.Ule
  let fune = fcmp Ollvm_ast.Une
  let funo = fcmp Ollvm_ast.Uno
  let ftrue = fcmp Ollvm_ast.True
  let ibinop b (t, op1) (_, op2) = (t, Ollvm_ast.INSTR_IBinop (b, t, op1, op2))
  ...
  let extractvalue agg idx = (fst agg, Ollvm_ast.INSTR_ExtractValue (agg, idx))
  ...
  let br1 (t, Ollvm_ast.VALUE_Ident branch) = Ollvm_ast.INSTR_Br_1 (t, branch)

  let switch sw default cases =
    let cases = List.map (fun (v, i) -> (v, ident i)) cases in
    Ollvm_ast.INSTR_Switch (sw, ident default, cases)

  let indirectbr tv til = Ollvm_ast.INSTR_IndirectBr (tv, List.map ident til)
...
module Block = struct
  ...
  let define fn args (instrs : block list) =
    ...
    let blocks =
      List.map (fun (id, instrs) -> (extract_name id, instrs)) instrs
    in
  ...
  let block id instrs =
    let _, id = Value.ident id in
    (id, instrs)
end

module Module = struct
  module Local = struct
    type t = { unnamed_counter : int; named_counter : (string * int) list }

    let empty = { unnamed_counter = 0; named_counter = [] }
    ...
    let local env t name =
      let env, name =
        ...
        | "" ->
            ...
            ({ env with unnamed_counter = i + 1 }, string_of_int i)
        | name -> (
            try
            ...
            with Not_found ->
              ({ env with named_counter = (name, 0) :: env.named_counter }, name)
            )
      in
  ...
  let set_data_layout m layout =
    {
      m with
      m_module =
        { m.m_module with m_datalayout = Ollvm_ast.TLE_Datalayout layout };
    }
  ...
  let lookup_declaration m name = List.assoc name m.m_module.m_declarations
  let lookup_definition m name = List.assoc name m.m_module.m_definitions
  ...
  let declaration m dc =
    let (Ollvm_ast.ID_Global name) = dc.Ollvm_ast.dc_name in
    {
      ...
      m_module =
        {
          m.m_module with
          m_declarations = (name, dc) :: m.m_module.m_declarations;
        };
    }
      ...
      m_module =
        {
          m.m_module with
          m_declarations = (name, dc) :: m.m_module.m_declarations;
          m_definitions = (name, df) :: m.m_module.m_definitions;
        };
    }
end
